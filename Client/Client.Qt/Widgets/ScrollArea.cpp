      ...
      _scrollMax(_connected->maximum())
{
    ...
    connect(_connected, SIGNAL(valueChanged(int)), this, SLOT(onValueChanged(int)));
    connect(_connected, SIGNAL(rangeChanged(int, int)), this, SLOT(onRangeChanged(int, int)));
...
void ScrollBar::onValueChanged(const int val)
{
    std::cout << "onValueChanged: " << val << std::endl;

    area()->onScrolled();
    updateBar();
}

void ScrollBar::onRangeChanged(const int first, const int second)
{
    std::cout << "onRangeChanged: " << first << ' ' << second << std::endl;
